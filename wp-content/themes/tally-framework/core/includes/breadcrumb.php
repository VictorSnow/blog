...
function breadcrumb_trail_get_buddypress_items($args = array()) 
{
	...
	if ( !empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
		...
		if (bp_loggedin_user_id() && bp_displayed_user_id() == bp_loggedin_user_id() || $bp->current_component == 'activity'){
			if (is_numeric($bp->current_action)) {
				...
				if($bp->current_action == 'just-me') {
					$trail_end = __('Personal', 'buddypress');
				} else {
					$trail_end = ucwords( str_replace('-', ' ', $bp->current_action ) );
				}
			}
		} else {
	 		$trail_end =  ucwords( $bp->current_component );
		...
		$trail[] = '<a href="'.  home_url().'/'. $bp->current_component .'/'. $bp->current_item .'" title="'.$bp->bp_options_title.'">'. $bp->bp_options_title .'</a>';
		...
		if(isset($bp->bp_options_nav[$bp->current_component][$bp->current_action])) {
		...
		if ($bp->action_variables) {
			$trail[] = '<a href="'.  home_url().'/'. $bp->current_component . '/' . $bp->current_item . '/' . $bp->current_action .'" title="'. ucwords($bp->current_action) .'">'. ucwords($bp->current_action) .'</a>';
			$trail_end = ucwords( str_replace('-', ' ', $bp->action_variables[0]) );
		}
                
	} else if ( $bp->is_directory ) { // this is a top level directory page
		if ( !$bp->current_component )
			$trail_end = sprintf( __( '%s Directory', 'buddypress' ), ucwords( BP_MEMBERS_SLUG ) );
		else
			 $trail_end = sprintf( __( '%s Directory', 'buddypress' ), ucwords( $bp->current_component ) );
		...
		$trail[] = '<a href="'.  home_url().'/'. $bp->current_component .'" title="'. ucwords($bp->current_component) .'">'. ucwords($bp->current_component) .'</a>';
		if ($bp->action_variables) {
			$trail[] = '<a href="'.  home_url().'/'. $bp->current_component . '/' . $bp->current_action .'" title="'. __( 'Create a Group', 'buddypress' ) .'">'. __( 'Create a Group', 'buddypress' ) .'</a>';
